...
void Segment::Render(glm::mat4 view, glm::mat4 proj) {
  ...
  model = T * R * S * PT * PS;//计算模型新的位置
  
  glUniformMatrix4fv(glGetUniformLocation(objectShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));//新的位置
  ...
  glUniform1f(glGetUniformLocation(objectShader.Program, "material.shininess"), 16.0f);//亮度
