...
public:
  ...
  // Eular Angles
  GLfloat Yaw;//绕摄像机上向量旋转
  GLfloat Pitch;//绕摄像机右向量旋转
  ...
  Camera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f),
         glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f),//初始化上向量为y轴 ，初始化前向量为向屏幕内
  ...
  {
    GLfloat velocity = this->MovementSpeed * deltaTime;//位移量
...
private:
  // Calculates the front vector from the Camera's (updated) Eular Angles 根据更新后的摄像机欧拉角计算前向量
  void updateCameraVectors()
  {
    // Calculate the new Front vector
    glm::vec3 front;
    front.x = cos(glm::radians(this->Yaw)) * cos(glm::radians(this->Pitch));//radian角度转弧度
